...
class PlayerPlane: SKSpriteNode {
    ...
    static func populate(at point: CGPoint) -> PlayerPlane {
        ...
        playerPlane.physicsBody?.categoryBitMask = BitMaskCathegory.player.rawValue
        playerPlane.physicsBody?.collisionBitMask = BitMaskCathegory.enemy.rawValue | BitMaskCathegory.powerUp.rawValue
        playerPlane.physicsBody?.contactTestBitMask = BitMaskCathegory.enemy.rawValue | BitMaskCathegory.powerUp.rawValue
    ...
    func performFly() {
        ...
        motionManager.startAccelerometerUpdates(to: OperationQueue.current!) { [unowned self] (data, error) in
            if let data = data {
                ...
                self.xAcceleration = CGFloat(acceleration.x) * 0.4 + self.xAcceleration * 0.3
            }
        }
        ...
        let planeSequence = SKAction.sequence([planeDirectionCheckAction, planeWaitAction])
